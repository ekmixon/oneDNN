...
def banner(year_from):
    ...
    banner_year = year_from if year_now == year_from else f'{year_from}-{year_now}'
    return '''\
/*******************************************************************************
...
def template(body, year_from):
    return f'{banner(year_from)}{body}'
...
def func_to_str_decl(enum, is_header=False):
    ...
    return f"const char {'DNNL_API ' if is_header else ''}*dnnl_{abbrev}2str({enum} v)"
...
for file_path in file_paths:
    with open(file_path, 'r') as f:
        ...
        banner_years.append(m[1].split('-')[0])
